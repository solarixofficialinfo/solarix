...
import {
  ...
  AlertTriangle, Navigation, CheckCircle2, ShieldCheck, Undo2, MapPin
} from "lucide-react";
import {
  toRad,
  toDeg,
 ...
 * 
 * Powered by Leaflet + High-Resolution Esri World Imagery (Global Satellite Tiles)
 * Integrates:
 * - Real-world satellite aerial tiles (zero API keys required, guaranteed 100% uptime)
 * - Map layer toggles: Satellite Imagery, Hybrid (Satellite + Road Labels), Street Map
 * - Point-and-click polygon roof boundary drawing directly on top of satellite imagery
 * - Visual vertex editing & dragging
...
const LiveSatelliteMap = forwardRef(function LiveSatelliteMap(
  ...
  useImperativeHandle(ref, () => ({
    getSnapshotDataUrl: () => {
      // Leaflet container rasterization
  ...
  useEffect(() => {
    ...
    try {
      ...
      const map = L.map(mapContainerRef.current, {
        ...
        maxZoom: 22,
        minZoom: 4,
        zoomControl: false, // custom zoom toolbar
      ...
      // Handle map clicks based on activeTool
      map.on("click", (e) => {
      ...
      setMapError("Map engine initialization failed: " + err.message);
    }
  ...
  // Keep window.__activeSolarTool updated for map click handler
  useEffect(() => {
  ...
  // Complete Polygon Drawing on Satellite Map
  const handleFinishDrawingRoof = useCallback(() => {
    ...
    // Convert (lat, lng) points to local Cartesian (x, y in meters) relative to origin
    const origin = { lat: activeDrawPoints[0].lat, lng: activeDrawPoints[0].lng };
    const cartesianPoints = activeDrawPoints.map((pt) => {
      ...
      return { x: Math.round(x * 100) / 100, y: Math.round(y * 100) / 100, lat: pt.lat, lng: pt.lng };
    });
  ...
  useEffect(() => {
    ...
    // 1. Draw Active In-Progress Drawing Points & Line
    if (activeDrawPoints.length > 0) {
      ...
      activeDrawPoints.forEach((p, idx) => {
        ...
        circleMarker.bindTooltip(`Point ${idx + 1}`, { permanent: true, direction: "top", className: "px-1 py-0 text-[10px] font-bold" });
      });
    }
    ...
    if (layers.roofBoundary && roofPolygon && roofPolygon.length >= 3) {
      ...
      if (layers.dimensions) {
        for (let i = 0; i < roofPolygon.length; i++) {
          ...
          const midLatLng = cartesianToLatLng(midX, midY);
    ...
    if (layers.obstacles && obstacles && obstacles.length > 0) {
      obstacles.forEach((obs) => {
        const ow = Number(obs.length || 1.5);
        const ol = Number(obs.width || 1.5);
        ...
        const obsPoly = L.polygon(obsLatLngs, {
          color: "#dc2626",
          weight: 2,
          fillColor: "#ef4444",
          fillOpacity: 0.45,
    ...
    if (layers.panels && panels && panels.length > 0) {
      panels.forEach((p, idx) => {
        ...
        const panelPoly = L.polygon(pLatLngs, {
          ...
          fillColor: isSelected ? "#2563eb" : "#0f2b59",
          fillOpacity: 0.9,
        }).addTo(panelGroup);
  ...
  return (
    <div className="relative w-full h-full min-h-[580px] rounded-2xl overflow-hidden bg-slate-900 border border-slate-700 shadow-xl select-none flex flex-col">
      ...
      {/* Map Load Error Diagnostic Banner */}
      {mapError && (
      ...
      {/* Drawing Instructions Overlay when in Draw Roof Mode */}
      ...
      {/* Bottom Live Metrics & Coordinates HUD */}
      <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between pointer-events-none z-10 gap-2">
        ...
        <div className="bg-slate-900/90 backdrop-blur-md px-3 py-1.5 rounded-xl border border-slate-700/80 shadow-lg text-[10.5px] text-slate-300 pointer-events-auto flex items-center gap-2">
          ...
          <span className="text-emerald-400 font-semibold flex items-center gap-1">
            <ShieldCheck className="w-3 h-3" /> Live Satellite Imagery
          </span>
        </div>
      </div>
    </div>
