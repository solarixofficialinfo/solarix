...
export function useEntitlements() {
  ...
  return {
    ...
    canWrite,
    daysRemaining,
    isFree: Boolean(subData?.is_free),
    accessType: subData?.access_type || (Boolean(subData?.is_free) ? "free_grant" : (isTrial ? "trial" : "paid")),
    extraDays: Number(subData?.extra_days || 0),
    effectiveExpiryDate: subData?.effective_expiry_date || subData?.subscription_expires_at || subData?.trial_ends_at,
